...
**NOTE**: For working with floating point numbers with double precision, we have to use registers with even indices and know that the data is going to be stored in 64-bit format, or in 8 bytes, for example when we do "**ldc1 $f6, d2**", the data in d2 in RAM is going to be loaded to $**f6"** and $**f7**. Other arithmetic operations like mul.d/s and div.d/s can be used on these data types.

For banching in FP realm, there are following instructions, first just we have to compare and set or reset based on that a flag and then use another instruction to use.

```python
	c.eq.d # compare equal
	c.eq.s 

	c.le.d # less or equal
	c.le.s 

	c.lt.d # less than
	c.lt.s 

	# branch if FP condition flag is 0
	bc1f label

	# branch if FP condition flag is 1
	bc1t label
```
